...
class TestInvokerBranchDelete(unittest.TestCase):
    ...
    def test_invoker_00(self, mock_run):
        response = make_invoker_cmd_call(invoker=RequestTypeToInvoker[backend_pb2.CMD_BRANCH_DEL],
                                         sandbox_root=self._sandbox_root,
                                         req_type=backend_pb2.CMD_BRANCH_DEL,
        ...
        expected_cmd = "cd {0} && mir branch -d {1}".format(self._mir_repo_root, self._singleton_op)
